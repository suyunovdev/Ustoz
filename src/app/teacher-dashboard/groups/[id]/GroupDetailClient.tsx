...
export default function GroupDetailClient({ groupId }: Props) {
  ...
  const handleArchive = () => {
    updateMut.mutate(
      ...
      {
        onSuccess: () =>
          toast.success(g.status === 'archived' ? t('teacher.groupDetailActivated') : t('teacher.groupDetailArchived')),
  ...
  return (
    <div className="max-w-4xl mx-auto p-6">
        ...
        <div className="flex items-start justify-between gap-4 mb-3">
          ...
          <div className="flex items-center gap-2 shrink-0">
            ...
            >
              ...
              {t('teacher.groupDetailBroadcast')}
            </button>
            <button
            ...
            >
              {g.status === 'archived' ? t('teacher.groupDetailActivate') : t('teacher.groupDetailArchive')}
            </button>
          </div>
        </div>
      </div>
      ...
      <div className="flex items-center justify-between mb-3">
        <h2 className="text-lg font-medium">{t('teacher.groupDetailMembers')} ({memberList.length})</h2>
        <button
        ...
        >
          ...
          {t('teacher.groupDetailAddMember')}
        </button>
      </div>
      ...
      {members.isLoading ? (
        ...
        <div className="text-center py-12 bg-muted/30 rounded-md text-sm text-muted-foreground italic">
          {t('teacher.groupDetailNoMembers')}
        </div>
      ) : (
          ...
          {memberList.map((m) => (
            ...
            >
              ...
              <button
                ...
                aria-label={t('teacher.groupDetailRemoveBtn')}
              >
        ...
        <BroadcastModal
          ...
          onSubmit={(input) =>
            broadcastMut.mutate(input, {
              onSuccess: ({ sent }) => {
                toast.success(`${sent} ${t('teacher.groupBroadcastSent')}`);
      ...
      {pendingRemove && (
        <ConfirmModal
          open={true}
          title={t('teacher.groupDetailRemoveTitle')}
          message={`${pendingRemove.fullName} ${t('teacher.groupDetailRemoveMessage')}`}
    ...
    >
      ...
      >
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-lg font-heading font-semibold">{t('teacher.addMembersTitle')}</h3>
        ...
        <input
          ...
          placeholder={t('teacher.addMembersSearch')}
          className="w-full px-3 py-2 border border-border rounded-md text-sm mb-3"
        />

        <p className="text-xs text-muted-foreground mb-2">
          {t('teacher.addMembersSelected')}: {selected.size} / {t('teacher.addMembersFreeSlots')}: {maxSlots}
        </p>
        ...
        <ul className="max-h-72 overflow-y-auto border border-border rounded-md divide-y divide-border">
          {filtered.length === 0 ? (
            <li className="p-3 text-sm text-center text-muted-foreground italic">
              {allStudents.isLoading ? t('teacher.addMembersLoading') : t('teacher.addMembersNoMatch')}
            </li>
          ) : (
            filtered.map((s) => {
        ...
        <div className="flex items-center justify-end gap-2 mt-4 pt-3 border-t border-border">
          ...
          >
            ...
            {selected.size > 0 ? `${selected.size} ${t('teacher.addMembersSubmit')}` : t('teacher.addMembersSubmitDefault')}
          </button>
        </div>
      </div>
    </div>
...
function BroadcastModal({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (title.trim().length < 2) return toast.error(t('teacher.groupBroadcastTitleMin'));
    if (message.trim().length < 2) return toast.error(t('teacher.groupBroadcastMessageMin'));
  ...
  return (
    ...
    >
      ...
      >
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-lg font-heading font-semibold">{t('teacher.groupBroadcastTitle')}</h3>
        ...
        <div className="space-y-3">
          <div>
            <label className="block text-sm font-medium mb-1">{t('teacher.groupBroadcastTitleLabel')}</label>
            <input
              type="text"
              value={title}
          ...
          <div>
            <label className="block text-sm font-medium mb-1">{t('teacher.groupBroadcastMessageLabel')}</label>
            <textarea
              value={message}
