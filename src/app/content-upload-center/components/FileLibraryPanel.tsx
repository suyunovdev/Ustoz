...
const FileLibraryPanel = ({ files, onFileDelete }: FileLibraryPanelProps) => {
  const getStatusBadge = (status: string) => {
    const statusConfig = {
      pending: { label: 'Kutilmoqda', color: 'bg-warning/10 text-warning', icon: 'ClockIcon' },
      approved: { label: 'Tasdiqlangan', color: 'bg-success/10 text-success', icon: 'CheckCircleIcon' },
      rejected: { label: 'Rad etilgan', color: 'bg-destructive/10 text-destructive', icon: 'XCircleIcon' },
      revision_requested: { label: 'Qayta ko\'rib chiqish', color: 'bg-secondary/10 text-secondary', icon: 'ArrowPathIcon' }
    };
  ...
  return (
    <div className="bg-card rounded-md shadow-warm p-4 space-y-4 h-fit sticky top-4">
      ...
      <div className="flex items-center justify-between pb-3 border-b border-border">
        <h3 className="text-lg font-heading font-semibold text-foreground">Fayl kutubxonasi</h3>
        <Icon name="FolderIcon" size={24} className="text-primary" />
      </div>

      {/* Status Summary */}
      <div className="space-y-2">
        <h4 className="text-sm font-medium text-foreground">Status bo'yicha</h4>
        <div className="space-y-1">
          <div className="flex items-center justify-between p-2 bg-warning/5 rounded-md">
            <div className="flex items-center space-x-2">
              ...
              <span className="caption text-foreground">Kutilmoqda</span>
            </div>
          ...
          <div className="flex items-center justify-between p-2 bg-success/5 rounded-md">
            <div className="flex items-center space-x-2">
              ...
              <span className="caption text-foreground">Tasdiqlangan</span>
            </div>
          ...
          <div className="flex items-center justify-between p-2 bg-destructive/5 rounded-md">
            <div className="flex items-center space-x-2">
              ...
              <span className="caption text-foreground">Rad etilgan</span>
            </div>
      ...
      {/* Recent Files */}
      <div className="space-y-2">
        <h4 className="text-sm font-medium text-foreground">So'nggi fayllar</h4>
        <div className="space-y-2 max-h-96 overflow-y-auto">
          {files.length === 0 ? (
            <div className="text-center py-8">
              <Icon name="FolderOpenIcon" size={32} className="text-muted-foreground mx-auto mb-2" />
              <p className="caption text-muted-foreground">Fayllar yo'q</p>
            </div>
          ) : (
            files.map((file) => {
